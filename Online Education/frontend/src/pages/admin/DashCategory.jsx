...
const DashCategory = () => {
    ...
    const { courseType, loading } = useSelector(state => state.courseTypeAll);
    let data = [];
    data = (courseType !== undefined && courseType.length > 0) ? courseType : []

    //delete course by Id
    const deleteCourseCategoryById = (e, id) => {
        console.log(id)
    }
        ...
        {/*
            ...
            renderCell: (values) => (
                <Box sx={{ display: "flex", justifyContent: "space-between", width: "170px" }}>
                    ...
                    < Button onClick={(e) => deleteCourseCategoryById(e, values.row._id)} variant="contained" color="error">Delete</ Button>
                </Box>
            )
            */}
    ];
